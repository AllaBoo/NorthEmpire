<!DOCTYPE html>
<html lang="ru">

<head>
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link rel="stylesheet" href="../../styles/normalize.css">
  <link rel="stylesheet" href="../../styles/NE_style.css">
  <title>Осенние каникулы на Золотом кольце и в Москве | Экскурсионный тур для школьников</title>
  <meta name="description"
    content="Сборный экскурсионный тур по Золотому кольцу и Москве для мини-групп школьников от 5 человек. Проходит на осенних школьных каникулах. Экскурсионная программа тура построена с учётом особенностей школьников." />
  <meta name="Keywords" content="экскурсионный тур по Золотому кольцу и Москве для школьников, тур по Золотому кольцу и Москве на осенние каникулы" />
</head>

<body class="body">

<?php include("../../blocks/parts-of-pages/header.php"); ?>
 <div class="crutch"></div>
 <?php include("../../blocks/parts-of-pages/contacts.php"); ?>

 <section class="colour-block">
    <h1 class="heading__name">Осенние каникулы <span class="heading__span">на Золотом кольце и в Москве</span></h1>
    <p class="heading__type">Cборный экскурсионный тур для мини-групп школьников от 5+1</p>
    <p class="heading__date">4 дня: 26-30 октября 2020 года</p>
    <nav class="heading__nav">
      <ul class="heading__list">
        <li class="heading__list-item">
          <a href=" #program" class="heading__anchor">&nbsp;Программа тура&nbsp;</a>
        </li>
        <li class="heading__list-item">
          <a href=" #price" class="heading__anchor">&nbsp;Стоимость тура&nbsp;</a>
        </li>
        <li class="heading__list-item">
          <a href="https://north-empire.com/tour-programs/2020-autumn-Moscow-GoldenRing-school-tur-5i1.pdf" target="_blank"
            class="heading__anchor">Скачать тур в PDF</a>
        </li>
        <li class="heading__list-item">
          <a href=" https://north-empire.com/tour-programs/2020-autumn-Moscow-GoldenRing-school-tur-5i1.doc"
            class="heading__anchor">Скачать тур в DOC</a>
        </li>
      </ul>
    </nav>
  </section>

  <div class="skew-cc"></div>
  <section class="white-block">
  <h2 class="title">Что такое сборный тур 5+1? Кому он Подходит?</h2>
  <?php include("../../blocks/parts-of-pages/5i1-description.php"); ?>
  </section>
  <div class="skew-c"></div>

  <section class="colour-block">
    <h2 class="title">Преимущества сборного тура</h2>
    <div class="cards">
    <?php include("../../blocks/advantages/fix-price.php"); ?>
    <?php include("../../blocks/advantages/teachers.php"); ?>
    <?php include("../../blocks/advantages/train.php"); ?>
    <?php include("../../blocks/advantages/GoldenRing.php"); ?>
    <?php include("../../blocks/advantages/bus.php"); ?>
    <?php include("../../blocks/advantages/meet.php"); ?>
    <?php include("../../blocks/advantages/meel.php"); ?>
    <?php include("../../blocks/advantages/full-day.php"); ?>
    <?php include("../../blocks/advantages/wheather.php"); ?>
    <?php include("../../blocks/advantages/care.php"); ?>
    </div>
  </section>
  <a name="program"></a>
  <div class="skew-cc"></div>
  <section class="white-block">
  <h2 class="title">Краткая информация о туре</h2>
  <div class="tour-list__icons">
    <?php include("../../blocks/icons/sborny_tur.php"); ?>
    <?php include("../../blocks/icons/autumn.php"); ?>
    <p class="card__text card__text_center">&laquo;Осенние каникулы на&nbsp;Золотом кольце и&nbsp;в&nbsp;Москве&raquo;</p>
    <?php include("../../blocks/icons/5i1.php"); ?>
    <?php include("../../blocks/icons/comfort.php"); ?>
  </div>
  <div class="info">
    <div class="card card_half">
      <div class="card">
        <img class="card__image" src="../../images/icons/calendar.jpg" alt="Календарь">
        <div class="card__description">
          <h4 class="card__title">Даты:</h4>
          <p class="card__text">26-30 октября 2020 года</p>
          <p class="card__text">5 дней</p>
        </div>
      </div>
      <div class="card">
        <img class="card__image" src="../../images/icons/spoon-fork.png" alt="Питание">
        <div class="card__description">
          <h4 class="card__title">Питание:</h4>
          <p class="card__text">2-разовое</p>
        </div>
      </div>
      <div class="card">
        <img class="card__image" src="../../images/icons/bus.png" alt="Автобус">
        <div class="card__description">
          <h4 class="card__title">Автобус:</h4>
          <p class="card__text">каждый день</p>
        </div>
      </div>
      <div class="card">
        <img class="card__image" src="../../images/icons/picking.png" alt="Экскурсии">
        <div class="card__description">
          <h4 class="card__title">Экскурсии:</h4>
          <ul class="info__museums-list">
            <li class="info__museum">автобусная обзорная по Москве</li>
            <li class="info__museum">пешеходная по Красной площади</li>
            <li class="info__museum">территория усадьбы Коломенское</li>
          </ul>
        </div>
      </div>
    </div>

    <div class="card card_half">
    <div class="card">
      <img class="card__image" src="../../images/icons/map.png" alt="Объекты показа">
      <div class="card__description">
        <h4 class="card__title">Музеи:</h4>
        <ul class="info__museums-list">
            <li class="info__museum">Кремль</li>
            <li class="info__museum">Третьяковская галерея</li>
            <li class="info__museum">усадьба Царицыно (Большой дворец)</li>
            <li class="info__museum">Успенский собор во Владимире</li>
            <li class="info__museum">Суздальский Кремль</li>
            <li class="info__museum">музей деревянного зодчества в Суздале</li>
            <li class="info__museum">церковь Покрова-на-Нерли</li>
          </ul>
      </div>
      </div>
    </div>

  </div>
</section>

  <div class="skew-c"></div>
  <main class="colour-block">
    <h2 class="title">Программа тура</h2>
    <p class="tour__mark">Выберите день и вам откроется подробная программа экскурсионного тура:</p>

    <details class="day">
      <summary class="day__summary summary">
      26 октября (понедельник)
        <h3 class="day__sightseeing">Обзорная экскурсия по Москве, Красная площадь, Кремль</h3>
      </summary>
      <?php include("../../blocks/excursions/meet_railway.php"); ?>
      <?php include("../../blocks/excursions/breakfast_cafe.php"); ?>
      <?php include("../../blocks/excursions/city-tour.php"); ?>
      <?php include("../../blocks/excursions/Red-Square.php"); ?>
      <?php include("../../blocks/excursions/Kremlin.php"); ?>
      <?php include("../../blocks/excursions/dinner.php"); ?>
      <?php include("../../blocks/excursions/hotel.php"); ?>
    </details>

    <details class="day">
      <summary class="day__summary summary">
      27 октября (вторник)
        <h3 class="day__sightseeing">Владимир</h3>
      </summary>
      <?php include("../../blocks/excursions/breakfast_hotel.php"); ?>
      <?php include("../../blocks/excursions/meet_hotel.php"); ?>
      <?php include("../../blocks/excursions/transfer-to-railway.php"); ?>
      <div class="day__container">
        <img class="card__image card__image_small" src="../../images/icons/train-fast.png" alt="Пиктограмма поезд-экспресс">
        <div class="card__description card__description_big">
          <p class="card__title">Переезд поездом «Ласточка»</p>
          <p class="card__text">до города Владимир.</p>
        </div>
        <p class="card__text spacing">Продолжительность:
          <span class="card__span">~ 2 часа.</span></p>
      </div>
      <?php include("../../blocks/excursions/Bogolubovo.php"); ?>
      <?php include("../../blocks/excursions/Nerl.php"); ?>
      <?php include("../../blocks/excursions/Vladimir-city-tour.php"); ?>
      <?php include("../../blocks/excursions/dinner.php"); ?>
      <?php include("../../blocks/excursions/Vladimir-street.php"); ?>
      <?php include("../../blocks/excursions/hotel.php"); ?>
      </details>

    <details class="day">
      <summary class="day__summary summary">
      28 октября (среда)
        <h3 class="day__sightseeing">Суздаль</h3>
      </summary>
      <?php include("../../blocks/excursions/breakfast_hotel.php"); ?>
      <?php include("../../blocks/excursions/meet_hotel.php"); ?>
      <?php include("../../blocks/excursions/Suzdal-kremlin.php"); ?>
      <?php include("../../blocks/excursions/Suzdal-wood.php"); ?>
      <?php include("../../blocks/excursions/Suzdal-concert.php"); ?>
      <?php include("../../blocks/excursions/Suzdal-cloister.php"); ?>
      <?php include("../../blocks/excursions/dinner.php"); ?>
      <div class="day__container">
        <img class="card__image card__image_small" src="../../images/icons/bus.png" alt="Пиктограмма автобус">
        <div class="card__description card__description_big">
          <p class="card__title">Трансфер на вокзал во Владимир.</p>
          <p class="card__text">Путевая информация.</p>
        </div>
        <p class="card__text spacing">Продолжительность:
          <span class="card__span">45 минут.</span></p>
      </div>
      <div class="day__container">
        <img class="card__image card__image_small" src="../../images/icons/train-fast.png" alt="Пиктограмма поезд-экспресс ">
        <div class="card__description card__description_big">
          <p class="card__title">Переезд поездом «Ласточка»</p>
          <p class="card__text">в Москву.</p>
        </div>
        <p class="card__text spacing">Продолжительность:
          <span class="card__span">~ 2 часа.</span></p>
      </div>
      <div class="day__container">
        <img class="card__image card__image_small" src="../../images/icons/bus.png" alt="Пиктограмма автобус">
        <div class="card__description card__description_big">
          <p class="card__title">Трансфер от вокзала до гостиницы в Москве.</p>
          <p class="card__text">Путевая информация. Размещение.</p>
        </div>
        <p class="card__text spacing">Продолжительность:
          <span class="card__span">1 час.</span></p>
      </div>
    </details>

    <details class="day">
      <summary class="day__summary summary">
      29 октября (четверг)
        <h3 class="day__sightseeing">Третьяковская галерея</h3>
      </summary>
      <?php include("../../blocks/excursions/breakfast_hotel.php"); ?>
      <?php include("../../blocks/excursions/meet_hotel.php"); ?>
      <?php include("../../blocks/excursions/metro-included.php"); ?>
      <?php include("../../blocks/excursions/Zamoskvoreche.php"); ?>
      <?php include("../../blocks/excursions/Tretyakovka.php"); ?>
      <?php include("../../blocks/excursions/dinner.php"); ?>
      <?php include("../../blocks/excursions/finish_center.php"); ?>
    </details>

    <details class="day">
      <summary class="day__summary summary">
      30 октября (пятница)
        <h3 class="day__sightseeing">Царицыно, Коломенское</h3>
      </summary>
      <?php include("../../blocks/excursions/breakfast_hotel.php"); ?>
      <?php include("../../blocks/excursions/meet_hotel.php"); ?>
      <?php include("../../blocks/excursions/bus.php"); ?>
      <?php include("../../blocks/excursions/Tsaritsino-Palace.php"); ?>
      <?php include("../../blocks/excursions/Kolomenskoe.php"); ?>
      <?php include("../../blocks/excursions/dinner.php"); ?>
      <?php include("../../blocks/excursions/transfer-to-railway.php"); ?>
    </details>

    <p class="card__text">Вероятно изменение порядка посещения музеев при сохранении программы.</p>

  </main>

  <div class="skew-cc"></div>

  <section class="white-block">
    <a name="price"></a>
    <h2 class="title">Стоимость тура в Москву</h2>
    <p class="price__subtitle">На 1 человека в российских рублях</p>
    <p class="price__subtitle">(действительна только для групп школьников от 5+1)</p>

    <table class="table-desktop_only">
      <thead class="persons">
        <tr>
          <th><p class="table__text">Размещение в Москве на выбор +
          <span class="table__span">«Золотое Кольцо» 3* г. Владимир, номера с удобствами, завтрак шведский стол</span></p>
        </th>
          <th>
          Цена
          </th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td>
            <p class="table__text"><span class="table__span">«ВАЛС» 2*</span> блоки 2+2, 2+1, 2+3</p>
            <p class="table__text"><span class="table__span">метро «Павелецкая»</span> 7&nbsp;минут пешком</p>
            <p class="table__text">завтрак шведский стол</p>
          </td>
          <td>22500</td>
        </tr>
        <tr>
          <td>
            <p class="table__text"><span class="table__span">«Максима Заря» 3*</span> номера с удобствами</p>
            <p class="table__text"><span class="table__span">метро «Окружная»</span> 5&nbsp;минут пешком</p>
            <p class="table__text">завтрак шведский стол</p>
          </td>
          <td>23000</td>
        </tr>
        <tr>
          <td>
          <p class="table__text"><span class="table__span">«Аэрополис» 3*</span> номера с удобствами</p>
          <p class="table__text"><span class="table__span">метро «Аэропорт»</span> 15&nbsp;минут пешком</p>
          <p class="table__text">завтрак шведский стол</p>
        </td>
          <td>23000</td>
        </tr>
        <tr>
          <td>
            <p class="table__text"><span class="table__span">«Парк отель «Фили» 3*</span> номера с удобствами</p>
            <p class="table__text"><span class="table__span">метро «Багратионовская»</span> 15&nbsp;минут пешком</p>
            <p class="table__text">завтрак шведский стол</p>
          </td>
          <td>23300</td>
        </tr>
      </tbody>
    </table>
    <details class="price__spoiler">
      <summary class="price__summary summary">В стоимость входит:</summary>
      <ul class="price__includes">
        <li class="price__service">Проживание в выбранной гостинице по количеству ночей. Внимание! 3-е место в номере
          еврораскладушка!</li>
        <li class="price__service">Питание – завтраки и обеды каждый день, включая день заезда.</li>
        <li class="price__service">Работа экскурсовода каждый день по программе.</li>
        <li class="price__service">Работа автобуса каждый день по программе, кроме 4 дня.</li>
        <li class="price__service">Встреча на ж/д вокзале в 1-ый день с 07:00.</li>
        <li class="price__service">Трансфер на вокзал в день выезда.</li>
        <li class="price__service">Входные билеты и экскурсионное обслуживание в музеях в зависимости от выбранных
          дней:
          <ul class="price__includes">
            <li class="price__excursion">обзорная экскурсия по Москве</li>
            <li class="price__excursion">пешеходная экскурсия по Красной площади и Александровскому саду;</li>
            <li class="price__excursion">экскурсия по территории Московского Кремля с посещением одного собора;</li>
            <li class="price__excursion">экскурсия в Третьяковскую галерею;</li>
            <li class="price__excursion">пешеходная экскурсия по Замоскворечью;</li>
            <li class="price__excursion">экскурсия в усадьбу «Царицыно»;</li>
            <li class="price__excursion">пешеходная прогулка по территории усадьбы Коломенское;</li>
            <li class="price__excursion">обзорная экскурсия по г. Суздаль, экскурсии в Суздальский Кремль, Крестовые палаты, Рождественский собор, Спасо-Евфимиев монастырь и в музей деревянного зодчества; </li>
            <li class="price__excursion">обзорная экскурсия по г. Владимир: экскурсия в Успенский собор; экскурсия в п. Боголюбово и к ц. Покрова-на-Нерли;</li>
          </ul>
        </li>
      </ul>
    </details>
    <details class="price__spoiler">
      <summary class="price__summary summary">В стоимость не&nbsp;входит:</summary>
      <p class="price__service"><span class="table__span">Важно:</span> Все дополнительные услуги необходимо заказать
        и
        оплатить заранее, в
        Москве заказ и оплата не принимаются!</p>
      <ul class="price__not-includes">
        <li class="price__service">Услуги камеры хранения на вокзалах.</li>
        <li class="price__service">Билеты в музеи, не включенные в программу тура.</li>
        <li class="price__service">Каждый ужин с каждого человека в кафе города – от 450 руб./чел. </li>
        <li class="price__service">Проезд на общественном транспорте (каждая поездка 58 руб./чел.)</li>
        <li class="price__service">Посещение одного из московских цирков или театров (стоимость билетов от 550 до 5000
          руб.)</li>
        <li class="price__service">Ранняя встреча с 06:00 до 07:00 на вокзале и встреча при прибытии в аэропорт.</li>
        <li class="price__service">Дополнительное транспортное обслуживание.
        <?php include("../../blocks/extra-pay/bus_school.php"); ?>
        </li>
        <li class="price__service">Дополнительное проживание с завтраком.
        <?php include("../../blocks/extra-pay/hotel_school.php"); ?>
        </li>
      </ul>
    </details>
  </section>
  <div class="skew-c"></div>
  <section class="colour-block">
    <?php include("../../blocks/parts-of-pages/lead-form.php"); ?>
  </section>
  <?php include("../../blocks/parts-of-pages/footer.php"); ?>
</body>

</html>